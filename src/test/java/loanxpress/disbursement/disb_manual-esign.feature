Feature: S3 Document Upload Automation
  ...
  Background:
   ...
   @Sanity1 @tc0011
  Scenario: generating docs
    Given path '/disbursement/generate-docs'
  ...
  Scenario: Upload document with specified headers and body
    Given path '/disbursement/manual-esign'
     And request { application_id: '#(APP_ID)' , doc_link: 'cgcl-ilos-uat-ui/upload_docs/user/911039_110_axis-bank-file_2.pdf_1738228438469.pdf', doc_label: null } 
     When method POST
     Then status 200
     And print 'Response:', response
